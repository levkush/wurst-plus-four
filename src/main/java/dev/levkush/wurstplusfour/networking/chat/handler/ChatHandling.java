...
import dev.levkush.wurstplusfour.networking.chat.packets.ping.PingGetGlobal;
    ...
    public void chatLoop(){
        while (running) {
            try {
                if (mode.equals(ChatMode.GLOBAL)) {
                    if (System.currentTimeMillis() - lastPing == 1000) {
                        try {
                            ...
                            String[] data = getChatPacket.run(WurstplusFour.CLIENT_HANDLING.token);
                ...
                WurstplusFour.LOGGER.error("Exception in ChatLoop for IRC " + e);
            }
        }
    }
