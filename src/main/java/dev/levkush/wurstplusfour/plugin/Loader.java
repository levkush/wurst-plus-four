...
public class Loader {
    ...
    public Loader() {
        WurstplusFour.LOGGER.info("Loading Plugins");
        pluginDir = new File(WurstplusFour.CONFIG_MANAGER.pluginFolder);
    ...
    public void loadClasses() {
        if (pluginsFiles != null && pluginsFiles.length > 0) {
            ...
            classes.spliterator().forEachRemaining(name -> {
                try {
                    ...
                    WurstplusFour.LOGGER.info(ife[0].getName());
                    if (ife[0].equals(Plugin.class)) {
                        WurstplusFour.LOGGER.info("asd");
        ...
        if (!plugins.isEmpty()) {
            WurstplusFour.LOGGER.info(plugins.get(0).name());
            plugins.spliterator().forEachRemaining(Plugin::init);
        }
        if(!names.isEmpty()) {
            for (String name : names) {
                WurstplusFour.LOGGER.info("Loaded -> " + name);
            }
        }
        WurstplusFour.LOGGER.info("Loaded Plugins");
    }
