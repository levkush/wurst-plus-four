...
import dev.levkush.wurstplusfour.util.CrystalUtil;
...
public class Offhand extends Hack {
    ...
    private boolean Check32K() {
        ...
        for (Entity entity : mc.world.loadedEntityList) {
            if (entity != mc.player && WurstplusFour.FRIEND_MANAGER.isFriend(entity.getName()) && entity instanceof EntityPlayer && entity.getDistance(mc.player) < 7) {
    ...
    public static class StopPlayerMovement {
        ...
        public static void toggle(boolean on) {
            if (on) {
                WurstplusFour.EVENT_PROCESSOR.addEventListener(stopPlayerMovement);
            } else {
                WurstplusFour.EVENT_PROCESSOR.removeEventListener(stopPlayerMovement);
            }
        }
