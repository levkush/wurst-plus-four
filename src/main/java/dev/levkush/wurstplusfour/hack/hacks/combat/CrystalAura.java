...
import dev.levkush.wurstplusfour.hack.hacks.misc.AutoClip;
import dev.levkush.wurstplusfour.setting.type.*;
import dev.levkush.wurstplusfour.util.*;
...
public final class CrystalAura extends Hack {
    ...
    private final dev.levkush.wurstplusfour.util.elements.Timer crystalsPlacedTimer = new Timer();
    ...
    public EntityEnderCrystal getBestCrystal() {
        ...
        if (this.ezTarget != null) {
            WurstplusFour.KD_MANAGER.targets.put(this.ezTarget.getName(), 20);
    ...
    private boolean isPlayerValid(@NotNull EntityPlayer player) {
        ...
        if (WurstplusFour.FRIEND_MANAGER.isFriend(player.getName())) return false;
    ...
    private boolean setYawPitch(@NotNull EntityEnderCrystal crystal) {
        ...
        float yaw = angle[0];
        float pitch = angle[1];
        float spoofedYaw = WurstplusFour.ROTATION_MANAGER.getSpoofedYaw();
        if (Math.abs(spoofedYaw - yaw) > maxYaw.getValue() && Math.abs(spoofedYaw - 360 - yaw) > maxYaw.getValue() && Math.abs(spoofedYaw + 360 - yaw) > maxYaw.getValue()) {
            WurstplusFour.ROTATION_MANAGER.setPlayerRotations(Math.abs(spoofedYaw - yaw) < 180 ? spoofedYaw > yaw ? (spoofedYaw - maxYaw.getValue()) : (spoofedYaw + maxYaw.getValue()) : spoofedYaw > yaw ? (spoofedYaw + maxYaw.getValue()) : (spoofedYaw - maxYaw.getValue()), pitch);
            return false;
        } else {
            WurstplusFour.ROTATION_MANAGER.setPlayerRotations(yaw, pitch);
        }
        return true;
    }
