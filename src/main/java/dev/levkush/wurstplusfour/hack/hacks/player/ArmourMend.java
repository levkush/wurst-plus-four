...
public class ArmourMend extends Hack {
    ...
    private final dev.levkush.wurstplusfour.util.elements.Timer timer = new Timer();
    ...
    public void onTick() {
        ...
        if (this.taskList.isEmpty()) {
            ...
            if (this.mendingTakeOff.getValue() && InventoryUtil.holdingItem(ItemExpBottle.class) &&
                    ...
                    mc.world.playerEntities.stream().noneMatch(e ->
                            e != mc.player && !WurstplusFour.FRIEND_MANAGER.isFriend(e.getName())
        ...
        if (this.timer.passedMs((int) ((float) this.delay.getValue() * WurstplusFour.SERVER_MANAGER.getTpsFactor()))) {
            if (!this.taskList.isEmpty()) {
