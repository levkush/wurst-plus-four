...
import dev.levkush.wurstplusfour.event.events.BlockEvent;
...
import dev.levkush.wurstplusfour.util.elements.Pair;
    ...
    public void onUpdate() {
        ...
        if(!isActive && (packetBurrow.getValue() || packetCity.getValue())){
            ...
            if(packetBurrow.getValue() && !isActive){
                for (EntityPlayer entity : mc.world.playerEntities.stream().filter(entityPlayer -> !WurstplusFour.FRIEND_MANAGER.isFriend(entityPlayer.getName())).collect(Collectors.toList()))
                {
