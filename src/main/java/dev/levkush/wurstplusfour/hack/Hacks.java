package dev.levkush.wurstplusfour.hack;
...
import dev.levkush.wurstplusfour.util.ReflectionUtil;
...
public final class Hacks implements Globals {
    ...
    public Hacks() {
        ...
        try {
            ArrayList<Class<?>> modClasses = ReflectionUtil.getClassesForPackage("dev.levkush.wurstplusfour.hack.hacks");
    ...
    public final void onUnload() {
        ...
        this.hacks.forEach(WurstplusFour.EVENT_PROCESSOR::removeEventListener);
        ...
        if (customFont) {
            if (reverse) {
                List<Hack> list = this.getEnabledHacks().stream().sorted(Comparator.comparing(hack ->
                        WurstplusFour.GUI_FONT_MANAGER.getTextWidth(hack.getFullArrayString()))).collect(Collectors.toList());
