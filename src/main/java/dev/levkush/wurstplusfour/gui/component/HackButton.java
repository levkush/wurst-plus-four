...
import dev.levkush.wurstplusfour.gui.component.components.*;
...
import dev.levkush.wurstplusfour.setting.type.*;
...
public class HackButton extends Component {
    ...
    public void init(Hack mod, CategoryComponent parent, int offset, boolean update) {
        ...
        if (WurstplusFour.SETTINGS.getSettingFromHack(mod) != null) {
            for (Setting s : WurstplusFour.SETTINGS.getSettingFromHack(mod)) {
    ...
    public void renderComponent(int MouseX, int MouseY) {
        ...
        if (Gui.INSTANCE.customFont.getValue()) {
            WurstplusFour.GUI_FONT_MANAGER.drawStringWithShadow(this.mod.getName(), parent.getX() + WurstplusGuiNew.MODULE_FONT_SIZE, parent.getY() + this.offset + WurstplusGuiNew.MODULE_OFFSET + WurstplusGuiNew.HEIGHT / 2f - WurstplusGuiNew.FONT_HEIGHT, Gui.INSTANCE.fontColor.getValue().hashCode());
        } else {
            Globals.mc.fontRenderer.drawStringWithShadow(this.mod.getName(), parent.getX() + WurstplusGuiNew.MODULE_FONT_SIZE, parent.getY() + this.offset + WurstplusGuiNew.MODULE_OFFSET + WurstplusGuiNew.HEIGHT / 2f - WurstplusGuiNew.FONT_HEIGHT, Gui.INSTANCE.fontColor.getValue().hashCode());
        }
        if (this.isOpen) {
    ...
    public void renderToolTip(int mouseX, int mouseY){
        if(isHovered && Gui.INSTANCE.toolTips.getValue()){
            int length = WurstplusFour.GUI_FONT_MANAGER.getTextWidth(mod.getDescription());
            int height = WurstplusFour.GUI_FONT_MANAGER.getTextHeight();
            RenderUtil2D.drawRectMC(mouseX + 6, mouseY + 9, mouseX + length + 10, mouseY + height + 13, Gui.INSTANCE.toolTipColor.getValue().hashCode());
            WurstplusFour.GUI_FONT_MANAGER.drawStringWithShadow(mod.getDescription(), mouseX + 8, mouseY + 11, new Color(255,255,255).hashCode());
        }
