...
import dev.levkush.wurstplusfour.gui.hud.HudButton;
...
import dev.levkush.wurstplusfour.util.ColorUtil;
...
public class CategoryComponent implements Globals {
    ...
    public CategoryComponent(Hack.Category cat) {
        ...
        if(category.equals(Hack.Category.HUD)){
            for(HudElement element : WurstplusFour.HUD_MANAGER.getHudElements()){
            ...
            for (Hack mod : WurstplusFour.HACKS.getHacksAlp()) {
    ...
    public void renderFrame(int mouseX, int mouseY) {
        ...
        if (Gui.INSTANCE.customFont.getValue()) {
            WurstplusFour.GUI_FONT_MANAGER.drawStringWithShadow(category.getName(), this.x + WurstplusGuiNew.MODULE_FONT_SIZE, this.y + (this.height / 2f) - WurstplusGuiNew.FONT_HEIGHT, Gui.INSTANCE.fontColor.getValue().hashCode());
        } else {
