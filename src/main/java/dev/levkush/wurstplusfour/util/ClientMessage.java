...
import dev.levkush.wurstplusfour.hack.hacks.chat.ToggleMessages;
...
public class ClientMessage implements Globals {
    ...
    private static final String opener() {
        ...
        return ChatFormatting.GOLD + WurstplusFour.MODNAME + ChatFormatting.WHITE + " : " + ChatFormatting.RESET;
    }

    public static void sendToggleMessage(Hack hack, boolean enabled) {
        if(mc.world != null && mc.player != null) {
            if(WurstplusFour.HACKS.ishackEnabled("Toggle msgs")) {
    ...
    public static void sendIRCMessage(String message){
        sendMessage(ChatFormatting.WHITE + "["+ ChatFormatting.AQUA + "IRC - " + WurstplusFour.CHAT_HANDLING.mode + ChatFormatting.WHITE +"] " + ChatFormatting.RESET + message);
    }
