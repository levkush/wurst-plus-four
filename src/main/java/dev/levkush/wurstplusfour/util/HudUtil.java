...
public class HudUtil implements Globals {
    ...
    public static String getPingLine() {
        String line = "";
        int ping = WurstplusFour.SERVER_MANAGER.getPing();
        if (ping > 150) {
    ...
    public static String getTpsLine() {
        String line = "";
        double tps = MathsUtil.round(WurstplusFour.SERVER_MANAGER.getTPS(), 1);
        if (tps > 16) {
            line += ChatFormatting.GREEN;
        } else if (tps > 10) {
            line += ChatFormatting.YELLOW;
    ...
    public static void drawHudString(String string, int x, int y, int colour) {
        if (HudEditor.INSTANCE.customFont.getValue()) {
            WurstplusFour.GUI_FONT_MANAGER.drawStringWithShadow(string, x, y, colour);
        } else {
    ...
    public static int getHudStringWidth(String string) {
        if (HudEditor.INSTANCE.customFont.getValue()) {
            return WurstplusFour.GUI_FONT_MANAGER.getTextWidth(string);
        } else {
    ...
    public static int getHudStringHeight() {
        if (HudEditor.INSTANCE.customFont.getValue()) {
            return WurstplusFour.GUI_FONT_MANAGER.getTextHeight();
        } else {
    ...
    public static int getRightX(String string, int x) {
        ...
        if (HudEditor.INSTANCE.customFont.getValue()) {
            return x - WurstplusFour.GUI_FONT_MANAGER.getTextWidth(string);
        } else {
